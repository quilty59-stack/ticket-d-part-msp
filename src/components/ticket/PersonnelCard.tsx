  ...
  function PersonnelCard({ person, isAffected = false, badge }, ref) {
    ...
    return (
      <div
        ...
        {...attributes}
        className={cn(
          'flex flex-wrap items-center gap-x-2 gap-y-1 px-3 py-2 rounded-lg border bg-card text-card-foreground',
      ...
      >
        ...
        {person.session_code && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground flex-shrink-0 ml-auto">
            {person.session_code}
          </span>
        )}
        {badge && (
